...
- Java 17 or higher.
- Gson 2.13.2 - download the jar by running:
```bash
curl -L -o gson-2.13.2.jar https://repo1.maven.org/maven2/com/google/code/gson/gson/2.13.2/gson-2.13.2.jar
```

Place `gson-2.13.2.jar` in the project root directory.

### Running in IntelliJ IDEA:

1. Open the project in IntelliJ IDEA.
2. Add `gson-2.13.2.jar` as a library: **File -> Project Structure -> Libraries -> + -> select the jar**.
3. Set the main class to `Main.Main`.
4. Click **Run**.
...
1. Compile:
```bash
javac -cp gson-2.13.2.jar -d out $(find src -name "*.java")
```

2. Run:
```bash
java -cp out:gson-2.13.2.jar Main.Main
```
...
Once the app is running, click **Load from JSON...** and select a `.json` file describing the shapes to render.
...
- `AbstractDrawableShape` holds shared state (position, dimensions, options) and enforces the `draw(Canvas)` contract on subclasses.
- This avoids duplication and leaves rendering flexible.
